...
class SerialNumber(models.Model):
	...
	def default_get(self, fields):
		res = super(SerialNumber, self).default_get(fields)
	...
	@api.model
	def create(self, vals):
		if not vals.get('bom_id') or vals.get('production_id'):
	...
	@api.multi
	def write(self, vals):
		res = super(SerialNumber, self).write(vals)
...
class MrpPartRequestLine(models.Model):
	...
	part_request_id = fields.Many2one('serial.number', string="Material Request")
	sale_id = fields.Many2one('sale.order', string='SO Reference')	
